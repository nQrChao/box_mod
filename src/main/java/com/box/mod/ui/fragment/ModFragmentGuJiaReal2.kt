...
import com.box.mod.databinding.ModFragmentGujiaReal2Binding
...
import com.box.mod.ui.xpop.ModXPopupCenterGuJiaTips
...
class ModFragmentGuJiaReal2 :
    BaseTitleBarFragment<ModFragmentGuJiaReal2.Model, ModFragmentGujiaReal2Binding>() {
    ...
    override fun layoutId(): Int = R.layout.mod_fragment_gujia_real_2

    companion object {
        fun newInstance(): ModFragmentGuJiaReal2 {
            return ModFragmentGuJiaReal2()
        }
    }
    ...
    fun showTipPop(contentText: String, priceString: String) {
        if (!MMKVConfig.guJiaTips) {
            XPopup.Builder(context)
                ...
                .asCustom(
                    ModXPopupCenterGuJiaTips(
                        mActivity,
                    ) {
                        MMKVConfig.guJiaTips = it
                        showCommitKuaiSuPop(contentText, priceString)
                    })
                .show()
        } else {
            showCommitKuaiSuPop(contentText, priceString)
    ...
    fun showCommitKuaiSuPop(contentText: String, priceString: String) {
        XPopup.Builder(context)
            .dismissOnTouchOutside(false)
            .dismissOnBackPressed(false)
            .isDestroyOnDismiss(true)
    ...
    /**
     * 获取格式化的表单内容字符串
     * @return 拼接后的多行字符串，格式为 "标题：内容"
     */
    private fun getFormContentString(): String {
        getFormItemsList()
        // commitViewAdapter.data 是 List<ModValuationCommitBean>
        // joinToString 会遍历列表，将每个 item 转换为字符串，并用换行符连接
        return commitViewAdapter.data.joinToString(separator = "\n") { item ->
    ...
    /**
     * 提取表单数据并校验
     * @return 校验通过返回 MutableList，校验失败返回 null
     */
    private fun getFormItemsList(): MutableList<ModValuationCommitBean>? {
        // 1. 获取数据源
        ...
        for ((index, item) in formItemsList.withIndex()) {
            ...
            if (value.isNullOrBlank()) {
                validationError = "${item.title} 不能为空"
                errorIndex = index
                break // 发现第一个错误即停止
            }
            // 如果你需要那个 Map，可以在这里构建，但你要求返回 List，
            // 且原代码后续逻辑使用的是 adapter.data，所以这里仅做校验即可。
        }
        // 4. 处理错误 UI
        if (validationError != null && errorIndex != -1) {
            ...
            mHandler.postDelayed({
                val view = mDataBinding.recyclerView2.layoutManager?.findViewByPosition(errorIndex)
                view?.startAnimation(
                    AnimationUtils.loadAnimation(appContext, RC.anim.shake_anim)
                )
            }, 100)
            return null // 校验失败，返回空
        }
        // 5. 校验通过，返回列表
    ...
    inner class ProxyClick {
        ...
        fun confirm() {
            ...
            // 2. 获取并校验表单数据
            // 如果返回 null，说明校验失败（UI逻辑已在方法内处理），直接 return 中断
            val validFormList = getFormItemsList() ?: return
            // 3. 图片数量检查
            ...
            // 4. 提交数据
            mViewModel.valuationCommitList.value = validFormList
